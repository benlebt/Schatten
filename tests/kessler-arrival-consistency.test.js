...
const image = images.find((entry) => entry.file === 'spedition-schmidt-moabit.webp');
assert(image && image.innen === true, 'Spedition image must be declared as interior');
assert.strictEqual(image.dayFile, 'spedition-schmidt-moabit-day.webp', 'Spedition must expose a daylight variant');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/kessler/hinterhof-sybelstrasse-day.webp', 'initial Kessler scene must select the daylight courtyard asset');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/kessler/kessler-wohnung-charlottenburg-night.webp', 'Kessler apartment at night must select the true night apartment asset');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/kessler/cafe-wien-night.webp', 'Cafe Wien at night must keep the true night cafe asset even during transitional prose');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/kessler/cafe-wien-night.webp', 'Cafe Wien text fallback must select the true night cafe asset');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/wegener/volkspolizei-hans-beimler-day.webp', 'explicit Roth police station location must beat Spedition prose mentions');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/wessel/charite-notaufnahme-day.webp', 'explicit Charite location must beat stale Opel/Revier anchors and prose references');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/wegener/imbiss-bei-trude.webp', 'explicit Trude location must beat Sybelstrasse clue prose and stale courtyard anchors');
...
assert.strictEqual(visualElements['kessler-scene-image'].attrs.src, 'assets/scenes/kessler/hinterhof-sybelstrasse-night.webp', 'courtyard night scene must select the true night asset');
...
const interiorRepairStart = html.indexOf('function repairCanonicalInteriorProse');
const interiorRepairEnd = html.indexOf('function _abschlussOrtOhneFestesTelefon', interiorRepairStart);
const interiorRepairContext = {
  engineCurrentLocation: { name: 'Spedition Schmidt Moabit' },
  diag: () => {}
};
...
const speditionMismatch = {
  szene: 'Tetzlaff steht allein in der fensterlosen Lagerhalle und sortiert Frachtpapiere auf einem wackeligen Metalltisch.'
};
