...
assert(/const _erledigtOhneSonderaktion = target\.erledigt\s*&& !_hauptuiInformantMitOffenemHinweis\(target\)\s*&& !_hauptuiRomanceAktion\(target\)\s*&& !_hauptuiHeilerAktion\(target\)\s*&& !\(typeof _hauptuiKlientenberichtOffen === 'function' && _hauptuiKlientenberichtOffen\(target\)\)\s*&& !target\.sonderAktion;/.test(html), 'only finished targets without romance, treatment, an open informant clue, a pending client report, or a special action may be disabled');
