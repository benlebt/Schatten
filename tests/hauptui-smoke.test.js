...
assert(html.includes('? CUSTODY_RELEASE_SCENE_IMAGE')
    && html.includes(': (istGewahrsam ? CUSTODY_SCENE_IMAGE'),
  'release and custody state must override ordinary location image matching');
...
assert(html.includes('${_abschlussAnzeige(_offeneLageZeile, false)}'), 'the solved screen must surface and normalize unresolved bound-person cleanup if the AI skips it');
...
assert(html.includes('romanticTension = Math.min(5, romanticTension + 1);'), 'an accepted romantic gesture must advance romance once per day and person');
...
const threadApproach = byText(threadContainer, 'Bestechen / Gegenleistung anbieten');
assert(threadApproach, 'Spedition thread must open Tetzlaff\'s conversation approaches');
threadApproach.onTap();
const threadExecute = all(threadContainer).find((element) => element.className === 'hauptui-execute');
assert(threadExecute && visibleText(threadExecute).includes('Bestechen / Gegenleistung anbieten'), 'Spedition thread must require an explicit Tetzlaff approach before execution');
...
const vossApproach = byText(container, 'Normal und höflich reden');
assert(vossApproach, 'selecting Voss must expose explicit conversation approaches');
vossApproach.onTap();
...
execute.onTap();
assert(context.caseProgress.uiAuditLog.some((event) => event.kind === 'AUSFUEHREN-BUTTON' && /Normal und höflich reden/.test(event.label)), 'execute clicks must enter the selected social approach in the UI audit');
...
for (const location of clueLocations) {
  ...
  personButton.onTap();
  assert(byText(container, 'Normal und höflich reden'), location.place + ' person mode must offer explicit conversation approaches');
...
assert(html.includes('if (target && target.erledigt && !bezwungen && !(informantMitHinweis && informantMitDirekterSozialwahl)) {'), 'finished peaceful conversations need a dedicated action gate while stale informant clues remain recoverable');
...
assert(/const _erledigtOhneSonderaktion = target\.erledigt\s*&& !_hauptuiInformantMitOffenemHinweis\(target\)\s*&& !_hauptuiRomanceAktion\(target\)\s*&& !_hauptuiHeilerAktion\(target\)\s*&& !\(typeof _hauptuiKlientenberichtOffen === 'function' && _hauptuiKlientenberichtOffen\(target\)\)\s*&& !\(typeof _hauptuiOffenesRettungsziel === 'function' && _hauptuiOffenesRettungsziel\(target\)\)\s*&& !\(typeof _hauptuiZielpersonTransportAktion === 'function' && _hauptuiZielpersonTransportAktion\(target\)\)\s*&& !target\.sonderAktion;/.test(html), 'only finished targets without romance, treatment, an open informant clue, a pending client report, a pending rescue or delivery, or a special action may be disabled');
assert(sourceOf('_hauptuiZielHinweis').includes('_hauptuiZielpersonTransportAktion'),
  'transported physical targets must advertise their pending handoff instead of looking spoken out');
...
assert(html.includes("(target.anzeigename || target.name) + (target.anzahl > 1 ? ' ×' + target.anzahl : '')"), 'inventory target label must show the grouped quantity while respecting guarded display names');
